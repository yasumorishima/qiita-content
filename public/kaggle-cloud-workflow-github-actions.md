...
Kaggleのコンペティションに参加したいけれど、ローカルPCにGPUがない、ディスク容量が足りない、出先のデバイスからも作業したい――そんな悩みを抱えている方は多いのではないでしょうか。

この記事では、**ノートブックの編集からKaggleへのアップロードまでをGitHub Actionsで自動化**するワークフローを紹介します。
...
API操作を行う前に、以下がブラウザ上で完了しているか確認してください。未完了の場合も一律で403エラーが発生します。

- コンペ規約への同意（Rules → Accept）
- 携帯電話番号によるアカウント認証
...
非ASCII文字を含むログは`kaggle kernels output`でWindowsのcp932エラーになります。API直叩きで回避できます：
