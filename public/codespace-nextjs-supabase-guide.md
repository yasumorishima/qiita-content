...
> **Note:** `--machine basicLinux32gb`で十分。Supabase起動 + Next.js devサーバーが問題なく動く。
...
> **Warning:** これをやらないと、ブラウザからSupabaseのAPIに到達できない。
...
> **Note:** DB操作は**Codespace内のターミナル**から行うので、こちらは`127.0.0.1`（localhost）でOK。ブラウザを経由しないため。
...
> **Note:** 停止し忘れると無料枠を消費するので注意。GitHubの無料枠はPersonalアカウントで月120コア時間。`basicLinux32gb`（2コア）なら60時間使える。
