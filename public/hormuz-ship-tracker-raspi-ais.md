...
tags:
  ...
  - FastAPI
private: false
updated_at: '2026-03-16T09:10:55+09:00'
...
![ペルシャ湾全域の船舶分布（3月中旬時点）](https://raw.githubusercontent.com/yasumorishima/hormuz-ship-tracker/7188e89/docs/snapshot_latest.png)
...
## 4日間の蓄積データから見えてきたこと（3月18日追記）

4日間の連続収集で43,000件以上の位置データが蓄積されました。

### 船舶密度ヒートマップ

![船舶密度ヒートマップ](https://raw.githubusercontent.com/yasumorishima/hormuz-ship-tracker/master/docs/heatmap.png)

*左: ペルシャ湾全域。Dubai付近に集中。右: 海峡拡大。「AIS DEAD ZONE」が受信不能エリア。下段: 港域別・船籍国別・船種別の分布。*

| 項目 | 数値 |
|---|---|
| 位置データ（クリーン） | 36,000件 |
| 異常データ（除外済み） | 7,300件（17%） |
| ユニーク船舶 | 384隻 |
| 海峡通過（確認済み） | 0隻 |
| Dubai / Jebel Ali ゲート通過 | 61回 |

### タイムラプス — 24時間の船舶移動

![船舶移動タイムラプス](https://raw.githubusercontent.com/yasumorishima/hormuz-ship-tracker/master/docs/timelapse.gif)

*24時間の船舶移動アニメーション。前後のデータポイントから線形補間し、陸地横断をフィルタリング済み。*

### AIS異常値の正体

データの約17%に異常が含まれていました。

| 異常の種類 | 件数 | 原因 |
|---|---|---|
| 速度 102.3ノット | 約3,200件 | AISプロトコルの「速度データなし」センチネル値（10bit 0x3FF） |
| 速度 40〜99ノット | 約4,100件 | 沿岸AIS受信局のデコードエラー |

48ノット付近の異常は、2026-03-16 07:00 UTCに4隻が同時にホルムズ海峡の同一座標に出現するパターンでした。受信局1局の誤動作で、船舶が実際にそこにいたわけではありません。この異常データが初期のtransit検知で41件の偽陽性を生んでいましたが、速度40ノット以上を除外するフィルタで解決しています。

ダッシュボードでは異常値のある船舶を赤い点線マーカーで表示し、「DATA QUALITY WARNING」をポップアップに表示するようにしました。

### リプレイ機能

`/replay`でLeaflet.jsベースのアニメーションリプレイが利用可能です。再生・速度変更（0.25x〜16x）・タイムラインスクラブに対応しています。

## 制約と注意点

- **陸上AISの限界**: aisstream.io無料プランは陸上受信局ベース。海峡中央部のカバレッジは限られる
- **AIS速度102.3ノット**: AIS仕様の「速度利用不可」センチネル値。フィルタが必要
- **速度40〜99ノットの受信局グリッチ**: 沿岸受信局のデコードエラーで偽データが発生。transit検知では40ノット以上を除外
- **データ収集期間**: 継続収集中。長期トレンド分析には更なる蓄積が必要

## まとめ

aisstream.ioの無料APIとRaspberry Pi 5で、ペルシャ湾全域の船舶をリアルタイムに収集・分析するシステムを構築しました。4日間で43,000件以上のデータを収集し、ヒートマップ・タイムラプス・データ品質分析まで実装しています。

統計レポートは6時間ごとに自動更新されています。

**[最新の統計レポート（自動更新）](https://github.com/yasumorishima/hormuz-ship-tracker/blob/master/docs/STATS.md)** / **[リポジトリ](https://github.com/yasumorishima/hormuz-ship-tracker)**
